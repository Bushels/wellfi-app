...
- Obsidian-configured wells can now show a survey-driven trajectory plate that separates surveyed stations from projected TD rows.
...
- `src/lib/directionalSurvey.ts`: directional survey parsing, projected-tail handling, and minimum-curvature interpolation.
...
- `scripts/generate_well_trajectory_plate.ts`: standalone SVG trajectory plate export from a survey CSV.
...
- `.claude/skills/`: portable repo skills for repeated validation or workflow execution (auto-loaded by Claude Code when launched from this project).
...
- Keep WellFi-local skills in `.claude/skills/` (Claude Code auto-discovery) — the canonical project location.
